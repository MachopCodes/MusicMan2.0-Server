...
router.patch('/reviews/:id', (req, res, next) => {
  const id = req.params.id; const reviewData = req.body; const profileId = reviewData.profileId
...
router.delete('/reviews/:id', (req, res, next) => {
  ...
  Profile.findById(profileId).then(handle404).then(profile => {
      const review = profile.reviews.id(id); review.remove(); return profile.save()
    }).then(() => res.sendStatus(204)).catch(next)
})
